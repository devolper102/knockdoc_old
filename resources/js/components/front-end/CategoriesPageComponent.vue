                    ...
                    <div id="dc-twocolumns" class="dc-twocolumns dc-haslayout">
                        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 float-left">
                            ...
                            <!--<div id="dc-docpostslider" class="dc-docpostslider owl-carousel">
                                <VueSlickCarousel v-bind="settings" v-if="this.doctors.length > 0" ref="carousel">
                                    <div v-for="(doc, index) in this.doctors">
                                        <div class="item">
                                            <div class="dc-docpostholder">
                                                <figure class="dc-docpostimg" style="height: 150px !important;">
                                                ...
                                                <div class="dc-docpostcontent">
                                                    <a v-if="savedDoctor != null && savedDoctor.includes(doc.id)" :class="'doctor-'+doc.id" href="javascript:void(0);" class="dc-like dc-clicksave" @click="wishlist(doc.id, savedDoctor, 'saved_doctors')">
                                                        <i class="fa fa-heart"></i>
                                                    </a>
                                                    <a v-else href="javascript:void(0);" class="dc-like" :class="'doctor-'+doc.id" @click="wishlist(doc.id, savedDoctor, 'saved_doctors')">
                                                        <i class="fa fa-heart"></i>
                                                    </a>
                                                    <div class="dc-title" >
                                                        <a href="#" class="dc-docstatus" v-for="(spec, index) in doc.specialities">{{spec.title}}</a>
                                                        <h3>
                                                            <a :href="'/profile/'+doc.slug">{{doc.first_name+" "+doc.last_name}}</a>
                                                        </h3>
                                                        <ul class="dc-docinfo">
                                                            <li><em>{{doc.profile.sub_heading}}</em></li>
                                                            <li>
                                                                <span class="dc-stars">
                                                                    <span :style="'width: {{doc.feedbacks.avg_rating / 5 * 100}}'"></span>
                                                                </span>
                                    ...
                                    <div class="dc-docpostholder" v-for="(doc, index) in this.doctors" v-if="index < doctorsToShow">
                                        <div class="dc-docpostcontent">
                                            ...
                                            <div class="dc-doclocation dc-doclocationvtwo">
                                                ...
                                                <span><i class="ti-thumb-up"></i>{{doc.profile.votes}} </span>
                                    ...
                                    <div class="card-header" :id="'heading-'+index">
                                        <h5 class="mb-0">
                                            <a class="collapsed" role="button" data-toggle="collapse" :href="'#collapse-'+index" aria-expanded="false" :aria-controls="'collapse-'+index">
                                                <span style="font-family: 'Open Sans',Arial,Helvetica,sans-serif;" v-html="JSON.parse(doctors[index].profile.faqs)[0]['question']"></span>
                                            </a>
                                        </h5>
                                    </div>
                                    <div :id="'collapse-'+index" class="collapse" data-parent="#accordion" :aria-labelledby="'heading-'+index">
                                        <div class="card-body">
                                            <span style="font-family: 'Open Sans',Arial,Helvetica,sans-serif;" v-html="JSON.parse(doctors[index].profile.faqs)[0]['answer']"></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
...
<script>
  ...
  export default {
    ...
    props: ['diseaseTitle','diseaseSlug','diseaseFlag','diseaseDescription',],
    data() {
      return {
        infoWindow: {
    ...
    methods: {
      ...
      getAllDoctors: function () {
        ...
        axios.get(APP_URL + '/getAllDoctors')
          .then(function (response) {
            // console.log(response.data.doctors[2].order_meta[0].meta_value)
            self.doctors = response.data.doctors;
              if (self.doctors.length > 0 ) {
                  self.doctors.forEach(function (sandwich, index) {
                      let permission = false;
                      if (sandwich.diseases.length > 0) {
                          let diseases = sandwich.diseases;
                          diseases.forEach(function (sandwichDisease) {
                              if(sandwichDisease.slug == self.diseaseSlug) {
                                  permission = true;
                              }
                          });
                          if (permission === false) {
      ...
      diseaseCheck: function (userData, diseaseSlug) {
        ...
        if(userData.diseases.length != 0){
          let diseases = userData.diseases;
