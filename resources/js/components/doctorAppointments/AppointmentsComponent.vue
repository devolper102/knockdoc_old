      ...
      <div class="dc-haslayout">
        <div class="dc-dashboardbox dc-apointments-wrap">	
          ...
          <div class="dc-recentapoint-holder" v-if="this.appointments.count > 0">
            <appointment-list
                    :appointment_list="appointments.appointment"
                    :paginated_index="commentsToShow"
            ></appointment-list>
...
export default {
  props: ['type', 'selected_appointment_date', 'appointment_type','appointmentsData'],
  data: function () {
      return {
        url:APP_URL,
        pageCount: 0,
  ...
  methods: {
    ...
    getAppointments(date) {
      var self = this;
      var id = window.location.href.substring(41)
      if (id) {
        axios.post(APP_URL + '/'+self.type+'/get-appointments', {date:date, id:id})
