        ...
        <div class="form-group form-group-label gallery gallery-image-area">
            <div class="la-gallery-image-holder">
                @foreach ($galleries as $key => $gallery)
                    @php 
                        if (file_exists(public_path('uploads/users/'.$id.'/gallery/images/'.$gallery))) {
                                $document_size = File::size(Helper::publicPath().'/uploads/users/'.$id.'/gallery/images/'.$gallery);
                        } else {
                                $document_size = 0;
                        }
                    @endphp
                    <div class="la-gallery-image">
                        <div class="dc-uploadingbox dz-image-preview dz-processing dz-success dz-complete" id="attachment-item-{{$key}}">
                            <figure><img src="{{asset('/uploads/users/'.$id.'/gallery/images/'.$gallery)}}" alt=""></figure>
