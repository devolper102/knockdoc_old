...
@php
    ...
    $areas = App\Area::select('title', 'id')->get()->pluck('title', 'id')->toArray();
...
@endphp
<meta name="google-signin-client_id" content="233612827309.apps.googleusercontent.com">
<style>
    .social_icons a{
        float: left;
        margin-right: 10px;
                        ...
                        <div class="dc-joinforms">
                            <form method="POST" action="{{{ url('register/form-step1-custom-errors') }}}" class="dc-formtheme dc-formregister" @submit.prevent="checkStep1" id="register_form">
                                ...
                                <div v-if="step === 1">
                                    ...
                                    <div class="dc-haslayout" v-cloak>
                                        ...
                                        <div class="form-group form-group-half">
                                         ...
                                         <div class="form-group form-group-half">
                                            <input id="phone_number" type="text" pattern="03[0-9]{2}[0-9]{7}" maxlength="11" minlength="11" class="form-control" name="phone_number" placeholder="{{{ trans('lang.ph_num') }}}" value="{{ old('phone_number') }}" v-bind:class="{ 'is-invalid': form_step1.is_phone_number_error }" v-model="phone_number">
                                            <span class="help-block" v-if="form_step1.phone_number_error">
                                                <strong v-cloak>@{{form_step1.phone_number_error}}</strong>
                                            </span>
                                        </div>
                                ...
                                {{--<div class="dc-haslayout" v-if="step === 2" v-cloak>--}}
                                    ...
                                    <div class="form-group">
                                        <span class="dc-select">
                                            {!! Form::select('locations', $locations, null, array('id'=>'parent_loc', 'placeholder' => trans('lang.ph.select_locations'), 'v-bind:class' => '{ "is-invalid": form_step2.is_locations_error }')) !!}
                                    ...
                                    <div class="form-group">
                                        <span class="dc-select">
                                            {{--{!! Form::select('areas', $areas, null, array('id'=>'child_loc', 'placeholder' => trans('lang.ph.select_areas'), 'v-bind:class' => '{ "is-invalid": form_step2.is_areas_error }')) !!}--}}

                                            {!! Form::select('areas',[''=>'--- Select Area ---'],null,['id'=>'child_loc', 'class'=>'form-control']) !!}

                                            <span class="help-block" v-if="form_step2.areas_error">
                                                <strong v-cloak>@{{form_step2.areas_error}}</strong>
                                            </span>
                                        </span>
                                    </div>
                                    @endif
                                    ...
                                    @if(!empty($roles))
                                        <ul class="dc-startoption">
                                            ...
                                            @if($role['role_type']=='doctor' || $role['role_type']=='hospital' || $role['role_type']=='patient' || $role['role_type']=='laboratory')
                                    ...
                                    <div class="dc-checkboxholder">
                                        ...
                                        <div class="form-group">
                                            <button type="submit" class="dc-btn" @click.prevent="checkStep2('{{ trans('lang.email_not_config') }}')">{{{  trans('lang.btn_startnow') }}}</button>
                                        </div>
                                        <div class="form-group social_icons">
                                            ...
                                            {{--<div class="g-signin2" data-onsuccess="onSignIn"><a href="{{ url('/auth/redirect/google') }}" class="btn btn-primary"><i class="fab fa-google"></i> Google</a></div>--}}
                                        </div>
                                    </div>
                                </fieldset>
                                </div>
                            </form>
                            <div class="dc-joinformc" v-if="step === 2" v-cloak>
                            ...
                            <div class="dc-gotodashboard" v-if="step === 3" v-cloak>
                                <div class="dc-registerhead">
                                    <div class="dc-title">
...
@push('front_end_scripts')
    ...
    <script>
        jQuery('#child_loc').css('display', 'none');
        $('#parent_loc').on('change',  function() {
          axios.post(APP_URL + '/getChildLocations', {
            id: this.value,
          }).then(function (response) {
            var child = response.data.child

            var $dropdown = $("#child_loc")
            jQuery('.childOptions').remove()
            $.each(child, function() {
              $dropdown.append($("<option class='childOptions'/>").val(this.id).text(this.title));
            })
