<div class="dc-searchresult-grid dc-searchresult-list dc-searchvlistvtwo">
    @if (!empty($teams) && $teams->count() > 0)
        @foreach ($teams as $key => $hospital_team)
            @php
                $slots = json_decode($hospital_team->slots, true);
            ...
            @if ($team->user_id !== 'online')
                ...
                <div class="dc-docpostcontent">
                    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-8 float-left">
                    ...
                    <div class="col-xs-12 col-sm-12 col-md-5 col-lg-4 col-xl-4 float-left">
                        ...
                        @if (!empty($appointment_days))
                            <span><i class="ti-calendar"></i>
                                @foreach ($slots["days"] as $key => $day)
                                    <em class="dc-available">{{ ucfirst(html_entity_decode(clean($day))) }},</em>
                                @endforeach
                            </span>
                        @endif
                            <span>
                                <i class="ti-calendar"></i>
                                @if (in_array($current_day, $slots['days']))
                                {{$slots[$current_day]['start_time']}} - {{$slots[$current_day]['end_time']}}
                                @else
                                    <em class="dc-dayon" style="color: red">
                                        Not Available Today
                                    </em>
                                @endif
                            </span>
                            <span>
                                ...
                                {{ trans('lang.doctors_onboard') }}: {{ clean($hospital_obj->approvedTeams()->count()) }}
                            </span>
                            <span>
                                <i class="ti-wallet"></i>
                                @if (in_array($current_day, $slots['days']))
                                {{$slots[$current_day]['consultation_fee']}}
                                @else
                                    Not Available Today
                                @endif
                            </span>
                        <div class="dc-btnarea">
                            ...
                            @if (in_array($team->hospital->id, array($saved_hospitals)))
