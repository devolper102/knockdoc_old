...
class UserController extends Controller
{
 ...
 //$options = unserialize($order->metaValue('package'));
    ...
    public function roleListing()
    {
         ...
         if ($role === 'patient') {
             $users = User::role($role)->latest()->with('profile')->with('specialities')->with('feedbacks')
                 ->with('services')->with('location')->with('diseases')->with('orderMeta')->with('appointments_patient')
                 ->get();
         }
         ...
         return view('back-end.admin.users.users', compact('role_based_users','role'));
    }
    ...
    public function editUser($id)
    {
        if (!empty($id)) {
            ...
            $working_time = !empty($user_meta->working_time) ? $user_meta->working_time : '';
            $available_days = !empty($user_meta->available_days) ? json_decode($user_meta->available_days) : array();
            $leave = !empty($user_meta->leave) ? Helper::getUnserializeData($user_meta->leave) : array();
            $starting_price = !empty($user_meta->starting_price) ? $user_meta->starting_price : '';
            $memberships = !empty($user_meta->memberships) ? Helper::getUnserializeData($user_meta->memberships) : array();
    ...
    public function getAllDoctors(){
        $json = array();
        $doctors = User::role('doctor')->with('profile')->with('specialities')->with('feedbacks')->with('services')->with('location')->with('diseases')->with('orderMeta')->get();
        $hospitals = User::role('hospital')->with('profile')->with('specialities')->with('feedbacks')->with('services')->with('location')->with('diseases')->get();
