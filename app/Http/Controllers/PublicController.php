...
class PublicController extends Controller
{
    ...
    public function showProfile()
    {
        ...
        if (!empty($slug)) {
            ...
            $user = User::where('id', $user_id)->with('diseases')->with('specialities')->with('location')->with('services')->with('profile')->with('location')->with('feedbacks')->with('doc_teams')->with('teams')->with('appointments')->with('roles')->first();
            $avg_rating = Feedback::where('user_id', $user->id)->pluck('avg_rating')->first();
            $stars  = $avg_rating != 0 ? $avg_rating / 5 * 100 : 0;
            $saved_doctors = !empty(auth()->user()->profile->saved_doctors) ? json_decode(auth()->user()->profile->saved_doctors) : array();
            $saved_hospitals = !empty(auth()->user()->profile->saved_hospitals) ? json_decode(auth()->user()->profile->saved_hospitals) : array();
            $saved_articles = !empty(auth()->user()->profile->saved_articles) ? json_decode(auth()->user()->profile->saved_articles) : array();
            $specialities = !empty($user->profile->services) ? Helper::getUnserializeData($user->profile->services) : array();
            $experiences = !empty($user->profile->experiences) ? json_decode($user->profile->experiences, true) : array();
            $educations = !empty($user->profile->educations) ? json_decode($user->profile->educations, true) : array();
            $faqs = !empty($user->profile->faqs) ? (array) json_decode($user->profile->faqs) : array();
            ...
            $teams = Team::where('doctor_id', $user->id)->where('status', 'approved')->get();
            ...
            $usertype = Helper::getRoleTypeByUserID($user->id);

            $available_days = !empty(json_decode($user->profile->available_days))  ? json_decode($user->profile->available_days) : '';

                $script_data_doctor = [];
                $script_data_hospital = [];
                if($usertype == "doctor"){
                $script_data_doctor = Helper::doctorProfileStructure($user_id);
              }elseif ($usertype == "hospital") {
                $script_data_hospital = Helper::hospitalProfileStructure($user_id);
              }
            if ($role_type === 'doctor') {
    ...
    public function getSearchResult()
    {
        ...
        $AllUsers = User::with('diseases')->with('specialities')->with('location')->with('services')->with('profile')->with('location')->with('feedbacks')->with('doc_teams')->with('teams')->with('appointments')->with('roles');


        if (
            request()->route()->getName() === 'loc-area-spec-gen'
            ||
        ...
        if (request()->route()->getName() === 'specialityDetails')
        {
            $speciality = request()->route()->parameters['slug'];
            $type = 'doctor';
        }
        if (request()->route()->getName() === 'doctors-location-area')
        {
            $area = request()->route()->parameters['area'];
            $location = request()->route()->parameters['location'];
            $type = 'doctor';
        }
        if (request()->route()->getName() === 'hospitals-location-area')
        {
            $area = request()->route()->parameters['area'];
            $location = request()->route()->parameters['location'];
            $type = 'hospital';
        }
        if (request()->route()->getName() === 'specialityDetailsLocation'
            ||
            request()->route()->getName() === 'doctors-speciality-city')
        {
            $speciality = request()->route()->parameters['slug'];
            $location = request()->route()->parameters['location'];
            $type = 'doctor';
        }
        if (request()->route()->getName() === 'diseasesDetails')
        {
            $diseases = request()->route()->parameters['slug'];
            $type = 'doctor';
        }
        if (request()->route()->getName() === 'diseasesDetailsLocation')
        {
            $diseases = request()->route()->parameters['slug'];
            $location = request()->route()->parameters['location'];
            $type = 'doctor';
        }
        if (request()->route()->getName() === 'servicesDetails')
        {
            $diseases = request()->route()->parameters['slug'];
            $type = 'doctor';
        }
        if (request()->route()->getName() === 'servicesDetailsLocation')
        {
        ...
        if (request()->route()->getName() === 'speciality-by-location')
        {
        ...
        if (request()->route()->getName() === 'doctors-by-city')
        {
        ...
        if (request()->route()->getName() === 'hospitals-by-city')
        {
            $location = request()->route()->parameters['slug'];
            $type = 'hospital';
        }
        ...
        $display_get_ad_sec = !empty($sidebar) && !empty($sidebar['display_get_ad_sec']) ? $sidebar['display_get_ad_sec'] : '';
        $ad_content = !empty($sidebar) && !empty($sidebar['ad_content']) ? $sidebar['ad_content'] : '';
        $search = User::getSearchResult($AllUsers,$type, $keyword,$diseases, $location, $area, $service, $speciality, $gender, $order_by, $sort_by);
        ...
        $location=!empty($location) ? Location::where('slug',$location)->with('users')->with('specialities')->first()  : '';
        $service=!empty($service) ? Service::where('slug',$service)->with('speciality')->with('symptom')->with('diseases')->with('users')->with('faqsAssign')->first() : '';
        $diseases=!empty($diseases) ? Disease::where('slug',$diseases)->with('services')->with('treatment')->first() : '';
        $speciality=!empty($speciality) ? Speciality::where('slug',$speciality)->with('services')->with('services_symptom')->with('services_diseases')->with('locations')->with('user')->with('faqsAssign')->first() : '';

        if(!empty($users)) {
            foreach ($users as $key => $user) {
                $usertype = Helper::getRoleTypeByUserID($user->id);
                if($usertype == "doctor"){
                    $script_data_doctor[] = Helper::doctorProfileStructure($user->id);
                }
                elseif ($usertype == "hospital") {
                    $script_data_hospital[] = Helper::hospitalProfileStructure($user->id);
                }
            }
        }
        if ($speciality) {
        ...
        $script_data_localBusiness[] = Helper::localBusinessStructure($type, $speciality, $location, $diseases, $service);
            ...
            if (file_exists(resource_path('views/extend/front-end/search-results/index.blade.php'))) {
                return view(
                    ...
                    compact(
                        ...
                        'script_data_breadCrumbs',
                        'script_data_faqs',
                        'script_data_localBusiness',
                        'script_data_organization'
                    )
                );
            } else {
                return view(
