...
class SiteManagementController extends Controller
{
    ...
    public function homePageSettings()
    {
        if (Auth::user()) {
            ...
            $home_how_works = $this->settings::getMetaValue('subscribe_now_sec'); // Subscribe Now section
            ...
            $article_section = $this->settings::getMetaValue('data_protection'); //Data Protection Section
            ...
            $data_protection_img = !empty($article_section['hidden_data_protection_img']) ? $article_section['hidden_data_protection_img'] : '';
    ...
    {
        $json = array();
        $how_works_sec = !empty(SiteManagement::getMetaValue('subscribe_now_sec')) ? SiteManagement::getMetaValue('subscribe_now_sec') : array();
        ...
        $article_sec = !empty(SiteManagement::getMetaValue('data_protection')) ? SiteManagement::getMetaValue('data_protection') : array();
    ...
    public function getInnerPageSettings()
    {
        ...
        public function mapimg(Request $request)
    {
         $mapimg = new SiteManagement();
$path = Helper::PublicPath() .'/uploads/markers';
File::isDirectory($path) or File::makeDirectory($path, 0777, true, true);
        $mapimg ->meta_key = $request->map_img;
        $mapimg ->meta_value = $request->map_image;
        $mapimg ->save();
         Session::flash('message','Map Image Updated Successfully');  
         return Redirect::back();
    }
