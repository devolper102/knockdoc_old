...
class DoctorController extends Controller
{
    ...
    public function getUpdateDoctorAwards()
    {
        ...
        if (Auth::user()) {
            $userId = Session::get('user_admin_id');
                $awards = User::find($userId[0])->Profile->awards;
                $awards_list = !empty($awards) ? Helper::getUnserializeData($awards) : array();
                if (!empty($awards)) {
    ...
    public function addLocation(Request $request)
    {
        if (Auth::user()) {
            ...
            $doctor_info = Team::where('doctor_id', $user->id)->with('hospital')->paginate(4);
