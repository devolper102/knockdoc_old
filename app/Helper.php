            ...
            'diseases' => array(
              ...
              'faq' => array(
               ...
               // 'route' => 'feedbacks',
                ...
                'medicines' => array(
                'role' => array('admin'),
                'title' => trans('Medicines'),
                'link' => url('admin/medicine'),
                ...
                'childern' => array(
                    array(
                        'title' => trans('Doctor Feedback'),
                        'link' => url('admin/feedback'),
                        'route' => 'feedback',
                    ), array(
                        'title' => trans('Hospital Feedback'),
                        'link' => url('admin/feedback-hospital'),
                        'route' => 'feedback-hospital',
                    ),
                ),
            ),
...
//
                ...
                'childern' => array(
                    array(
                        'title' => trans('Visit Appointments'),
                        'link' => url('admin/visit-appointment'),
                        'route' => 'visit-appointment',
                    ), array(
                        'title' => trans('Online Appointments'),
  ...
  public static function getUserData($user_id)
    ...
    public static function hospitalAffiliation($affliated_hospital_name,$affliated_hospital_phone,$affliated_hospital_address,$affliated_hospital_image){
        $hospital_affilation = array (
            "hospitalAffiliation" =>array(
                ...
                "telephone" => $affliated_hospital_phone,
                "priceRange" => "PKR 1500",
                "address" => array(
                    "@type" => "PostalAddress",
    ...
    public static function rating($user_id){
        ...
        if(!empty($user_rating)){
             $rates = unserialize($user_rating->rating);
            foreach ($rates as $key => $rate)
                $ratings[] = html_entity_decode(clean($rate['rating']));
    ...
    public static function memberHospital($members){
        ...
        if(!empty($members)){
            foreach ($members as $key => $mem) {
                $user = User::with('profile')->where('id',$mem->doctor_id)->first();
                $city = Location::where('id',$user->location_id)->pluck('title')->first();
                $specialities = DB::table('user_service')->select('speciality')
                    ->where('user_id', $mem->doctor_id)->groupBy('speciality')->get()->pluck('speciality')->toArray();

                $services = DB::table('user_service')->select('service_id')
                    ->where('user_id', $mem->doctor_id)->groupBy('service_id')->get()->pluck('service_id')->toArray();
                if(!empty($specialities)){
                    $speciality = Speciality::whereIn('id',$specialities)->pluck('title')->toArray();

                }
                // $specialities_string = '"'. implode('","',$speciality).'"';
                
                $member[] = array(
                    "@type" => "Physician",
                    "name" => $user->first_name." ".$user->last_name,
                    "image" => config('app.url')."/uploads/users/".$user->id."/".$user->profile->avatar,
                    "address" => array(
                        "@type" =>"PostalAddress",
                        "streetAddress" => "",
                        "addressLocality" => "",
                    ...
                    "priceRange" => "PKR 1500",
                    "openingHours" => "",
                    "url" => config('app.url')."/profile/".$user->slug,
    ...
    public static function doctorProfileStructure ($user_id) {
        $user = User::with('profile')->where('id',$user_id)->first();
        $specialities = DB::table('user_service')->select('speciality')
                    ->where('user_id', $user_id)->groupBy('speciality')->get()->pluck('speciality')->toArray();
        $services = DB::table('user_service')->select('service_id')
                    ->where('user_id', $user_id)->groupBy('service_id')->get()->pluck('service_id')->toArray();
        
        $affliated_hospital_name = "";
        $affliated_hospital_phone = $user->phone_number ?? "Not provided";
        $affliated_hospital_address = $user->profile->address;
        $affliated_hospital_image = config('app.url')."/uploads/users/".$user_id."/".$user->profile->avatar;
        $days = [];
        $hospital_name = [];
        $hospital_phone = [];
        $hospital_address = [];
        ...
        if(!empty($specialities)){
            // foreach ($specialities as $spec) {
                $speciality[] = Speciality::whereIn('id',$specialities)->pluck('title');
            // }
        }
        ...
        $available_locations = Team::where('doctor_id', $user_id)->where('status', 'approved')->get();
        
        if(!empty($available_locations)){
        foreach ($available_locations as $key => $hospital) {
            
            $hospital_obj = User::with('profile')->where('id',$hospital->user_id)->first();
            if(!empty($hospital_obj)){
                ...
                if($hospital_obj->profile->avatar != null)
                    {
                        $hospital_image[] =  config('app.url')."/uploads/users/".$user_id."/".$hospital_obj->profile->avatar;
                    }
                }
            }
        }
        ...
        if(!empty($speciality)){
            $doctor_specialities = $speciality[0];
        }
        if(!empty($service)){
            $doctor_services = $service[0];
        }
        ...
        $script_data = array (
            ...
            "url" => url()->current()."/profile/".$user->slug,
            "telephone" => $user->phone_number,
            "priceRange" => "PKR 1500",
            "openingHours" => $available_days,
            "description" => $user->profile->description,
            "currenciesAccepted"=>"PKR",
            "image"=>array( 
               "@type" => "ImageObject",
               "url" => config('app.url')."/uploads/users/".$user_id."/".$user->profile->avatar,
                "height" => "170",
                "width" => "170",
            ),
            "hospitalAffiliation" =>self::hospitalAffiliation($affliated_hospital_name,$affliated_hospital_phone,$affliated_hospital_address,$affliated_hospital_image)["hospitalAffiliation"],
           ...
           "location" => array(
            ...
            "aggregateRating" => self::rating($user_id)["aggregateRating"]
        ); 
        return json_encode($script_data,JSON_UNESCAPED_SLASHES | JSON_PRETTY_PRINT);
    }
    public static function hospitalProfileStructure ($user_id) {
        $days = [];
        $service =[];
        $user = User::with('profile')->where('id',$user_id)->first();
        $services = DB::table('user_service')->select('service_id')
        ...
        $script_data = array (
            ...
            "url" => url()->current(),
            "telephone" => $user->phone_number,
            "currenciesAccepted"=>"Rs",
            "priceRange" => "PKR 1500",
            "openingHours" => $available_days,
            "image" => config('app.url')."/uploads/users/".$user_id."/".$user->profile->avatar,
            /*"image"=>array(
               "@type" => "ImageObject",
               "url" => config('app.url')."/uploads/users/".$user_id."/".$user->profile->avatar 
            ),*/
    ...
    public static function specialityStructure ($faqs) {
        ...
        foreach ($faqs as $faq) {
            ...
            array_push($script_data['mainEntity'] ,
            ["@type" => "Question",
                "name" => $question,
    ...
    public static function breadCrumbsStructure () {
        ...
        foreach ($breadcrumbs as $key => $segment) {
            ...
            array_push($script_data[0]['itemListElement'], [
                "@type" => "ListItem",
                "position" => $key + 1,
                "item" => "https://www.msnsoft.org".$segments,
                "name" => $name,
            ]);
        }
    ...
    public static function localBusinessStructure ($type, $speciality, $location, $diseases, $service) {

        $name = '';
        $image = '';
        $siteUrl = url('/');
        if ($speciality !== '' && $type !== '') {
            $name = $type . ' for ' . $speciality->title ;
            $image = $siteUrl.'/uploads/specialities/'.$speciality->image;
        }
        else {
            $image = $siteUrl. '/images/d-logo.png';
            $name = 'Knockdoc';
        }
        $script_data = array ();
            $script_data[] = array (
                ...
                "address" => array(
                    ...
                    "description" => "Gourmet, Airline Society ",
                    "postalCode" => "54000",
                    "name" => "Knockdoc",
    ...
    public static function organizationStructure() {
        ...
        $script_data = [
            ...
            "name" => "Knockdoc",
            "url" => $siteUrl,
            "logo" => $siteUrl."/uploads/settings/general/1592321417-final-sc.png",
            "description" => "
            Your next favourite appointment booking system. 
            Find and book appointments online with the best health professionals. 
            Get you health concerns solved faster by reaching out to doctors, hospitals and clinics in a much easier way!
            ",
            "foundingDate" => "2020",
            "address" => [
                "@type" => "PostalAddress",
