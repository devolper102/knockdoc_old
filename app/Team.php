...
class Team extends Model
{
    ...
    {
        return $this->belongsTo('App\User', 'user_id')->with('profile')->withDefault();
    }

    public function doctor()
    {
        return $this->belongsTo('App\User', 'doctor_id')->withDefault();
    }
    ...
    public function saveAppointmentLocation($request, $user_id = '')
    {
        ...
        if (!empty($request['slots'])) {
            ...
            $this->slots = json_encode($request_slots, true);
        }
        ...
        $profile->available_days = json_encode($slot['appointment_days'], true);
        $profile->save();
        return 'success';
    }
    ...
    public function updateAppointmentSlots($id, $request)
    {
        if (!empty($id)) {
            $team = Team::find($id);
            if (!empty($team)) {
                $stored_slots = json_decode($team->slots, true);
                $request_slots = array();
                ...
                $team->slots = json_encode($new_slots, true);
                $team->save();
                return 'success';
            } else {
                return 'error';
        ...
        if (!empty($id) && !empty($slot) && !empty($day)) {
            ...
            if (!empty($team)) {
                ...
                if (!empty($default_slots)) {
                    ...
                    $team->slots = json_encode($stored_slots, true);
                }
                $team->save();
                return 'success';
            } else {
    ...
    {
        if (!empty($doctor_id)) {
            $hospitals = Team::select('user_id')->where('doctor_id', $doctor_id)->where('status', 'approved')->get();
            $list = array();
            if (!empty($hospitals)) {
                foreach ($hospitals as $key => $hospital) {
                    ...
                    if ($hospital['user_id'] === 'online') {
                        $list[$key]['id'] = 'online';
                        $list[$key]['name'] = 'Online Booking Consultation';
                    }
                }
                return $list;
            } else {
