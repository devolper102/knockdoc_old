...
'meta_key','meta_value'];
    ...
    public static function saveHowItWorksSettings($request)
    {
        if (!empty($request)) {
            ...
            DB::table('site_managements')->insert(
                [
                    'meta_key' => 'subscribe_now_sec', 'meta_value' => serialize($request->except('_token')),
                    "created_at" => Carbon::now(), "updated_at" => Carbon::now()
                ]
            );
            return 'success';
    ...
    public static function saveHowWorksTabSettings($request)
    {
        if (!empty($request)) {
            ...
            if (!empty($request['hidden_how_it_works_img'])) {
                if (file_exists($old_path . '/' . $request['hidden_how_it_works_img'])) {
                    if (!file_exists($new_path)) {
                        File::makeDirectory($new_path, 0755, true, true);
                    }
                    $filename = time() . '-' . $request['hidden_how_it_works_img'];
                    rename($old_path . '/' . $request['hidden_how_it_works_img'], $new_path . '/' . $filename);
            ...
            if (!empty($how_works_tabs)) {
                ...
                DB::table('site_managements')->insert(
                     [
                    'meta_key' => 'how_work_tabs', 'meta_value' => serialize($request->except('_token','show_hwtabs')),
                    "created_at" => Carbon::now(), "updated_at" => Carbon::now()
                ]
                );
        ...
        if (!empty($request)) {
            ...
            if (!empty($request['hidden_benefits_online_img'])) {
                if (file_exists($old_path . '/' . $request['hidden_benefits_online_img'])) {
                    if (!file_exists($new_path)) {
                        File::makeDirectory($new_path, 0755, true, true);
                    }
                    $filename = time() . '-' . $request['hidden_benefits_online_img'];
                    rename($old_path . '/' . $request['hidden_benefits_online_img'], $new_path . '/' . $filename);
            ...
            if (!empty($benefits_online_tabs)) {
                ...
                DB::table('site_managements')->insert(
                     [
                    'meta_key' => 'benefits_online_profile', 'meta_value' => serialize($request->except('_token','show_onlinetabs')),
                    "created_at" => Carbon::now(), "updated_at" => Carbon::now()
                ]
                );
       ...
       // dd($request->all());
        if (!empty($request)) {
            ...
            if (!empty($request['hidden_benefits_onlinedoctor_img'])) {
                if (file_exists($old_path . '/' . $request['hidden_benefits_onlinedoctor_img'])) {
                    if (!file_exists($new_path)) {
                        File::makeDirectory($new_path, 0755, true, true);
                    }
                    $filename = time() . '-' . $request['hidden_benefits_onlinedoctor_img'];
                    rename($old_path . '/' . $request['hidden_benefits_onlinedoctor_img'], $new_path . '/' . $filename);
            ...
            if (!empty($benefits_onlinedoctor_tabs)) {
                ...
                DB::table('site_managements')->insert(
                     [
                    'meta_key' => 'benefits_online_doctor', 'meta_value' => serialize($request->except('_token','show_onlinedoctortabs')),
                    "created_at" => Carbon::now(), "updated_at" => Carbon::now()
                ]
                );
    ...
    public static function saveDataProtection($request)
    {
        ...
        if (!empty($request)) {
            ...
            if (!empty($request['hidden_data_protection_img'])) {
                if (file_exists($old_path . '/' . $request['hidden_data_protection_img'])) {
                    if (!file_exists($new_path)) {
                        File::makeDirectory($new_path, 0755, true, true);
                    }
                    $filename = time() . '-' . $request['hidden_data_protection_img'];
                    rename($old_path . '/' . $request['hidden_data_protection_img'], $new_path . '/' . $filename);
            ...
            DB::table('site_managements')->insert(
                [
                    'meta_key' => 'data_protection', 'meta_value' => serialize($request->except('_token')),
                    "created_at" => Carbon::now(), "updated_at" => Carbon::now()
                ]
            );
            return 'success';
    ...
    public static function saveSignUp($request)
    {
        ...
        if (!empty($request)) {
            ...
            if (!empty($request['hidden_sign_up_img'])) {
                if (file_exists($old_path . '/' . $request['hidden_sign_up_img'])) {
                    if (!file_exists($new_path)) {
                        File::makeDirectory($new_path, 0755, true, true);
                    }
                    $filename = time() . '-' . $request['hidden_sign_up_img'];
                    rename($old_path . '/' . $request['hidden_sign_up_img'], $new_path . '/' . $filename);
            ...
            DB::table('site_managements')->insert(
                [
                    'meta_key' => 'sign_up_section', 'meta_value' => serialize($request->except('_token')),
                    "created_at" => Carbon::now(), "updated_at" => Carbon::now()
                ]
            );
            return 'success';
